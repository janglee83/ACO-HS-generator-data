...
def generate_random_task(task_id):
    ...
    pre_tasks = random.sample(range(1, 501), random.randint(1, 6))
    ...
    requirements = {
        "certificates": [(certificate["abbreviation"]) for certificate in certificates_selected],
        "majors": majors_selected[0]['abbreviation'],
        "type_majors": majors_type_selected[0]['abbreviation']
    }
    ...
    return {
        ...
        "AffectedFactors": affected_factors
    }
...
random_tasks_data = [generate_random_task(i) for i in range(1, 501)]
