...
kpi_pd = pandas.read_csv('exported_data_medium/KPI_data.csv')
task_pd = pandas.read_csv('exported_data_medium/tasks.csv')
resource_pd = pandas.read_csv('exported_data_medium/resources.csv')
...
def get_resource_base_condition(certi, major, type_majors):
        ...
        if major == '':
            ...
            for item in json.loads(resource_data['Qualifications']):
                for key, value in item.items():
                    if major == '' or (key == 'Major' and major == value):
                        list_fit_major.append(resource_data)
    ...
    for resource_data in list_fit_major:
        if type_majors == '':
            ...
            for item in json.loads(resource_data['Qualifications']):
                for key, value in item.items():
                    if type_majors == '' or (key == 'Type_Major' and value == type_majors):
                        list_fit_major_and_type_major.append(resource_data)
...
csv_filename = 'exported_data_medium/performance_task_linkage.csv'
