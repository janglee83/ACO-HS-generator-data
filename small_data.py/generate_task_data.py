...
def generate_random_task(task_id):
    ...
    pre_tasks = random.sample(
        [num for num in range(1, 81) if num != task_id], 60)
...
random_tasks_data = [generate_random_task(i) for i in range(1, 81)]
...
df.to_csv("exported_data_medium/tasks.csv", index=False)
